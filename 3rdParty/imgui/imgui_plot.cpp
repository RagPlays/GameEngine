    ...
    PlotStatus Plot(const char* label, const PlotConfig& conf)
    {
        ...
        if (conf.values.count > 0)
        {
            ...
            if (conf.tooltip.show && hovered && inner_bb.Contains(g.IO.MousePos))
            {
                ...
                const size_t data_idx{ static_cast<size_t>(conf.values.offset + (v_idx % conf.values.count)) };
            ...
            if (conf.selection.show)
            {
                if (hovered)
                {
                    if (g.IO.MouseClicked[0])
                    {
                        ...
                        const int v_idx = cursor_to_idx(g.IO.MousePos, inner_bb, conf, x_min, x_max);
                        uint32_t start{ static_cast<uint32_t>(conf.values.offset + (v_idx % conf.values.count)) };
                        uint32_t end{ start };
                ...
                if (g.ActiveId == id)
                {
                    if (g.IO.MouseDown[0])
                    {
                        ...
                        uint32_t end{ static_cast<uint32_t>(conf.values.offset + (v_idx % conf.values.count)) };
                        if (end > start)
                        {
                            if (conf.selection.sanitize_fn)
                            {
                ...
                const float fSelectionStep{ static_cast<float>(1.0 / item_count) };
